...
const storeValue = <T>(key: string, value: T) => {
  try {
    ...
    console.warn(
      'Error storing value in useLocalStorage hook, value is not being persisted.',
      {
        error,
        value,
        key,
...
function retrieveValue<T>(key: string, initialValue?: T): T | undefined {
  try {
    ...
    console.warn(
      'Error retrieving value in useLocalStorage hook, initialValue is being used.',
      {
        error,
        initialValue,
        key,
...
export function useLocalStorage<T>(
  ...
  const [state, setState] = useState<T>(
    () => retrieveValue(key, initialValue) as T,
  )
  const currentLoadedKey = useRef(key)

  useDeepCompareEffect(
    () => storeValue(currentLoadedKey.current, state),
    [state],
  )
